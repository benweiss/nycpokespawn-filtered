...
module.exports = (tweetStream1, tweetStream2, logger, config) => {
  const twilioClient = twilio(config.twilio.accountSID, config.twilio.authToken);

  const happenings = createHappeningsEmitter(
    tweetStream1,
    tweetStream2,
    config.accountIDToFollow1,
    config.accountIDToFollow2,
    config.locations
  );

  logger.info("unownbot-filtered started" + (config.configLabel ? ` for ${config.configLabel}` : ""));
  logger.info("accountSID: " + `${config.twilio.accountSID}`);
  logger.info("accounts to follow: " + `${config.accountIDToFollow1}` + ", " + `${config.accountIDToFollow2}`);
...
- twitterReply: ${err.twitterReply}`);
  ...
  happenings.on("spawn within range", data => {
    ...
    const message = `${data.tweet.text}` + ` http:/` + `/maps.google.com/?q=${data.latitude},${data.longitude}&ll=${data.latitude},${data.longitude}&z=14`;
